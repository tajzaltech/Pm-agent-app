...
function BrandMark() {
  return (
    <span className="text-[15px] font-bold tracking-tight text-foreground">
      pm<span className="text-primary">agent</span>
    </span>
  );
}
    ...
    <div className="flex flex-col min-h-0">
      <div className="flex items-center justify-between px-3 py-1.5">
        ...
        <button
          type="button"
          onClick={handleNew}
          className="flex size-5 items-center justify-center rounded-md text-muted-foreground/60 transition-colors hover:bg-muted/60 hover:text-foreground"
          title="New task"
        >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-1.5 space-y-px">
        {tasks.length === 0 && (
          <p className="px-2 py-3 text-[11px] text-muted-foreground/50 text-center">
            No tasks yet
          </p>
        )}
        {tasks.map((task) => {
          ...
          return (
            ...
            >
              {isTicketTask ? (
                <Ticket size={13} weight="duotone" className="mt-0.5 shrink-0 text-primary/60" />
              ) : (
                <ChatText size={13} className="mt-0.5 shrink-0 opacity-50" />
              )}
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] leading-snug font-medium">
                  {task.title}
                </p>
                <p className="truncate text-[10px] leading-tight mt-0.5 opacity-50">
...
function DesktopSidebar() {
  ...
  return (
    <aside className="hidden lg:flex h-screen w-[220px] shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
      <div className="flex items-center px-5 h-[48px] shrink-0 border-b border-sidebar-border">
        <BrandMark />
      </div>

      <nav className="px-2.5 pt-3 space-y-px shrink-0">
        {NAV.map(({ href, label, Icon }) => {
          ...
          return (
            ...
            >
              ...
              {badge > 0 && (
                <span className="flex size-[18px] items-center justify-center rounded-full bg-primary text-[10px] font-semibold text-primary-foreground leading-none">
                  {badge}
                </span>
              )}
            </Link>
